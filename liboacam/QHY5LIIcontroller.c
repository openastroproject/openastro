...
_doSetResolution ( QHY_STATE* cameraInfo, int x, int y )
{
  ...
  cameraInfo->captureLength = cameraInfo->frameSize + QHY5LII_IMAGE_OFFSET;
...
_doReadExposure ( QHY_STATE* cameraInfo )
{
  ...
  // Great weirdness is here.  Sometimes the camera will return the amount
  // of data requested.  Sometimes it's five bytes short and the next
  // transfer request gets those remaining bytes.  It's utterly unclear to
  // me what should be going on, so I've just attempted to deal with the
  // mess as tidily as possible

  // first have a go at reading all the data

  // references to droppedFrames here could be mutexed, but it's not the
  // end of the world

  ret = _usbBulkTransfer ( cameraInfo, QHY_BULK_ENDP_IN,
      cameraInfo->xferBuffer, cameraInfo->captureLength, &xferred,
      USB2_TIMEOUT );
  if ( ret < 0 ) {
    fprintf ( stderr, "QHY5L-II::%s, usbBulkTransfer returns %d\n",
        __FUNCTION__, ret );
    cameraInfo->droppedFrames++;
    return -OA_ERR_CAMERA_IO;
  }
  // now, if we've read five bytes less than expected, read the remainder
  // into a dummy buffer as we won't use them anyhow

  if ( xferred != cameraInfo->captureLength ) {
    int ret2;
    unsigned int xfer2;
    uint8_t dummy[ QHY5LII_IMAGE_OFFSET ];

    if ( xferred == ( cameraInfo->captureLength - QHY5LII_IMAGE_OFFSET )) {

      oacamDebugMsg ( DEBUG_CAM_USB, "QHY5L-II: USB: %s, usbBulkTransfer "
          "off by %d, doing dummy read\n", __FUNCTION__, QHY5LII_IMAGE_OFFSET );

      ret2 =_usbBulkTransfer ( cameraInfo, QHY_BULK_ENDP_IN, dummy,
          QHY5LII_IMAGE_OFFSET, &xfer2, USB2_TIMEOUT );
      if ( ret2 < 0 ) {
        fprintf ( stderr, "QHY5L-II::%s, usbBulkTransfer(2) returns %d\n",
            __FUNCTION__, ret );
        cameraInfo->droppedFrames++;
        return -OA_ERR_CAMERA_IO;
      }
  ...
  oacamDebugMsg ( DEBUG_CAM_CMD, "QHY5L-II: command: %s() returns %d\n",
      __FUNCTION__, 0 );
