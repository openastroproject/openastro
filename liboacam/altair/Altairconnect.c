...
oaCamera*
oaAltairInitCamera ( oaCameraDevice* device )
{
  oaCamera*			camera;
...
*/
  ...
  if ( pthread_create ( &( cameraInfo->callbackThread ), 0,
      oacamAltaircallbackHandler, ( void* ) camera )) {
...
{
  camera->funcs.initCamera = oaAltairInitCamera;
  camera->funcs.closeCamera = oaAltairCloseCamera;

  camera->funcs.setControl = oaAltairCameraSetControl;
  ...
  camera->funcs.isStreaming = oaAltairCameraIsStreaming;

  camera->funcs.setResolution = oaAltairCameraSetResolution;
  camera->funcs.setROI = oaAltairCameraSetROI;
  camera->funcs.testROISize = oaAltairCameraTestROISize;
...
int
oaAltairCloseCamera ( oaCamera* camera )
{
  void*			dummy;
  ALTAIRCAM_STATE*	cameraInfo;
	int				j;
