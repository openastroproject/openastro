  ...
  do {
    ...
    do {
      ...
      if ( command ) {
        ...
        if ( command->callback ) {
					oaLogWarning ( OA_LOG_CAMERA, "%s: command has callback" );
        } else {
          pthread_mutex_lock ( &cameraInfo->commandQueueMutex );
          command->completed = 1;
    ...
    if ( streaming ) {
      ...
      if ( frameWait > ATIK_SERIAL_MAX_SHORT_EXPOSURE ) {
        ...
        if ( cameraInfo->write ( cameraInfo, ampOnCmd, 4 )) {
          oaLogError ( OA_LOG_CAMERA, "%s: write error on amp on", __func__ );
        }
        usleep ( 100000 );
      }
...
_processSetControl ( AtikSerial_STATE* cameraInfo, OA_COMMAND* command )
{
  ...
  oaControlValue*	val = command->commandData;
	...
	oaLogInfo ( OA_LOG_CAMERA, "%s ( %p, %p ): entered", __func__, command );
	oaLogDebug ( OA_LOG_CAMERA, "%s: control = %d", control );
