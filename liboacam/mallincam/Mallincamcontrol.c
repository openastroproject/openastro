...
int
oaMallincamCameraSetControl ( oaCamera* camera, int control,
...
int
oaMallincamCameraReadControl ( oaCamera* camera, int control,
    oaControlValue* val )
{
  OA_COMMAND		command;
  MALLINCAM_STATE*	cameraInfo = camera->_private;
...
int
oaMallincamCameraTestControl ( oaCamera* camera, int control,
    oaControlValue* valp )
{
  int32_t		val_s32;
  MALLINCAM_STATE*	cameraInfo = camera->_private;
...
int
oaMallincamCameraSetResolution ( oaCamera* camera, int x, int y )
{
  FRAMESIZE		s;
  OA_COMMAND		command;
  MALLINCAM_STATE*	cameraInfo = camera->_private;
...
int
oaMallincamCameraSetROI ( oaCamera* camera, int x, int y )
{
  FRAMESIZE		s;
  OA_COMMAND		command;
  MALLINCAM_STATE*	cameraInfo = camera->_private;
    ...
    void* (*callback)(void*, void*, int, void* ), void* callbackArg )
{
  ...
  oacamDebugMsg ( DEBUG_CAM_CTRL, "Mallincam: control: %s ( %p )\n",
...
int
oaMallincamCameraIsStreaming ( oaCamera* camera )
{
  MALLINCAM_STATE*  cameraInfo = camera->_private;
...
int
oaMallincamCameraStopStreaming ( oaCamera* camera )
{
  OA_COMMAND		command;
  int			retval;
  MALLINCAM_STATE*	cameraInfo = camera->_private;

  oacamDebugMsg ( DEBUG_CAM_CTRL, "Mallincam: control: %s()\n", __FUNCTION__ );
