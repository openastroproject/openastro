/*****************************************************************************
 *
 * Touptekcontroller.c -- Main camera controller thread
 *
...
void*
oacamTouptekcontroller ( void* param )
{
...
_processSetControl ( oaCamera* camera, OA_COMMAND* command )
{
  ...
  switch ( control ) {
    ...
    case OA_CAM_CTRL_MODE_AUTO( OA_CAM_CTRL_EXPOSURE_ABSOLUTE ):
      ...
      if ((( p_Toupcam_put_AutoExpoEnable )( cameraInfo->handle,
						valp->boolean )) < 0) {
...
_processSetROI ( oaCamera* camera, OA_COMMAND* command )
{
  ...
  if (!( camera->features.flags & OA_CAM_FEATURE_RAW_MODE )) {
    return -OA_ERR_INVALID_CONTROL;
  }
...
_setFrameFormat ( TOUPTEK_STATE* cameraInfo, int format )
{
  ...
  if ( !oaFrameFormats[ format ].monochrome ) {
    ...
    if ((( p_Toupcam_put_Option )( cameraInfo->handle, TOUPCAM_OPTION_RGB,
